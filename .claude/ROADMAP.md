...
- [x] Cost-runaway hard abort (2026-07-21) — closed the "no cost runaway protection" gap flagged repeatedly in the Weak Links assessment. `cost.py::_track_cost` (the single choke point every LLM call in the pipeline already routes through) now raises `CostRunawayError` if a run's cumulative spend exceeds `MAX_RUN_COST_USD` ($2.00 — ~4-6x a normal run's ~$0.30-0.50). Hard abort, not a warning, per explicit user decision (vs. a relative-to-recent-runs baseline or log-only).
