...
- [x] A8: Promote grounding check from monitoring to blocking — **2026-07-16: fixed the remaining pp-delta false-positive class** (`_is_verified_pp_delta` in `llm.py` — a number like "13.8 pp above the EA's 32.7%" is a correctly-computed difference of two already-cited numbers, not a fabrication; confirmed against a real production false positive from a wave-37 adhoc run). With that fix plus the earlier `n=`/wave-reference/`/10`-denominator fixes, `_check_numeric_grounding` is now wired to a hard `sys.exit`-equivalent abort in both `run_report.py::_check_grounding_blocking` and `run_adhoc_report.py::_check_grounding_blocking`, called right after each report's sections are finalized. Note: `_check_exec_provenance` (cross-section number citation) is a **separate check, still monitoring-only** — it caught a real cross-section mislabeling bug in the same calibration run (a number correctly cited in one section attributed to the wrong `section_id` in the exec summary) that needs its own fix before it's safe to block on.
