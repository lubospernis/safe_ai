...
def main() -> None:
    ...
    # Derive a distinct scores filename per input report so EN/SK runs don't
    # clobber each other's output. The two main-report names keep their exact
    # historical filenames (generate_report.yml and write_run_manifest.py both
    # hardcode "quality_scores.json") — anything else falls back to a general
    # "_<rest-of-stem>" suffix.
    ...
    try:
        ...
        # the CI job. The tier-1 style findings and chart check already ran
        # locally/independently of this call, so they're still reported
        # accurately even though the supervisor itself is unavailable.
